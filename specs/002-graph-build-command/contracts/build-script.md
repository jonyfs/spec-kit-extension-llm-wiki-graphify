...
```text
graph-build.(sh|ps1) check   [--path <p>] [--min-version <v>]
graph-build.(sh|ps1) scope   [--path <p>] [--exclude <glob>]...
graph-build.(sh|ps1) build   --confirmed [--path <p>] [--full] [--exclude <glob>]...
...
|---|---|---|
| 0 | `built`, `current`, `declined` | The run reached a legitimate conclusion |
...
| 5 | `dependency-too-old` | Resolves, but below the version floor |
...
2. Run `graphify --version`; parse `X.Y.Z`. Unparseable → exit 5, reporting what was
   found.
3. Compare against the floor (default `0.9.9`). Below → exit 5, reporting both versions.
4. Create no directory and write no file on any failure path (FR-004).

### `scope` (FR-005)

Report the resolved root, the file count, and the applied exclusions. Resolve the root and
reject any path escaping the project root before reporting. Writes nothing.

### `build` (FR-007 – FR-011, FR-019, FR-020)

1. Re-run the `check` sequence. The check is cheap and a stale earlier result is not
   evidence about now.
2. Acquire an exclusive lock for the project. Held → exit 6 without writing (FR-020).
3. Detect an incomplete previous graph. Present → exit 7 and require a `--full` run;
   never refresh from an incomplete state (FR-019).
4. Invoke `graphify update <path>`. This is the whole of the build; the script implements
   no extraction, clustering, or rendering (FR-009, Principle XVI).
5. Classify the tool's outcome: rebuilt → `built`; "No code-graph topology changes
   detected" → `current` (research R5); tool failure → `failed`.
6. Count from `graphify-out/graph.json`: entities from `nodes`, relationships from
   **`links`** — not `edges`, which does not exist and silently yields zero (research R4).
7. Break relationships down by `links[].confidence`, reproducing each label verbatim
   (FR-012).
8. On a refresh, report added, changed, and removed against the previous counts (FR-011).
9. Release the lock on every exit path, including interruption.
